...
#' @param w_P Weight for prevalence in the IM score (0.3).
#' @param w_A Weight for relative abundance in the IM score (0.3).
#' @param w_I Weight for the binding score in the IM score (0.4).
#'
#' @return A data frame with Species, Sample, Binding_score, Prevalence, Rel_ab, and IM_score.
#' @export
#'
#' @examples
#' # result <- calculate_im_score(otu_table, binding_score)
calculate_im_score <- function(otu_table,
                               binding_score,
                               w_P = 0.3,
                               w_A = 0.3,
                               w_I = 0.4) {
  ...
  # rename column names
  colnames(merged_data) <- c("Species", "Sample", "Binding score", "Prevalence", "Rel_ab", "IM_score")
