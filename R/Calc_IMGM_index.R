...
#'
#' This function takes two or more data frames, each containing an 'IM_Global_Score' column,
#' binds them row-wise, scales (z-score) the combined IM_Global_Score, and returns
#' the combined data frame with an added 'Z_score' column.
#'
#' @param ... Two or more data frames, each with an 'IM_Global_Score' numeric column.
#'
...
calculate_imgm_index <- function(...) {
  ...
  for (i in seq_along(dfs)) {
    if (!("IM_Global_Score" %in% colnames(dfs[[i]]))) {
      stop(paste0("Input data frame ", i, " does not have an 'IM_Global_Score' column."))
    }
  }
  ...
  z_scores <- scale(combined_df$IM_Global_Score)
